...
export default function TopProductsValueReport() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const response = await api.get('/produtos/maior-valor');
      const data: Product[] = response.data.map((item: any) => ({
        codigo_produto: item.codigo_produto,
        descricao_produto: item.descricao_produto,
        valor_total: parseFloat(item.valor_total),
        total_historico: parseFloat(item.total_historico),
        percentage: (parseFloat(item.valor_total) / parseFloat(item.total_historico)) * 100,
      }));
      setProducts(data);
    } catch (error) {
  ...
  const totalValue = products.reduce((sum, item) => sum + item.valor_total, 0);
  const totalHistorico = products.length > 0 ? products[0].total_historico : 0;
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center mb-8">
          ...
          <div className="flex items-center">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Top 10 Produtos em $</h1>
          </div>
        </div>

        <button
          onClick={() => router.push('/reports/top-products-quantity')}
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          ...
          <div className="bg-blue-50 rounded-xl p-4 flex justify-between items-center">
            ...
            <div>
              <p className="text-sm text-blue-600 font-medium">Valor Total Histórico</p>
              <p className="text-2xl font-bold text-gray-800 mt-1">R$ {totalHistorico.toLocaleString('pt-BR')}</p>
            </div>
          </div>
        </div>
            ...
            products.map((item, index) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-medium text-blue-600">R$ {item.valor_total.toLocaleString('pt-BR')}</p>
